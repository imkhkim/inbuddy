pipeline {
  ...
  stages {
    stage('.env, start.sh 복사') {
      steps {
        sh "rm -f ./scheduler/.env && cp ${env.DEV_METADATA}/scheduler/.env ./scheduler/.env"
        sh "rm -f ./scheduler/start.sh && cp ${env.DEV_METADATA}/scheduler/start.sh ./scheduler/start.sh"
      }
    }
