...
function CheckInCounterInfoPage() {
    ...
    const airlines = [{ "name": "ITA항공", "airlineCode": "AZ", "terminal": 1, "selfCheckIn": false },
    ...
    return (
        <div className="flex flex-col items-center justify-end w-full">
            ...
            <div className="my-5 w-full h-36 bg-white rounded-3xl shadow-lg flex flex-col items-center justify-center text-xl">
            ...
            <div className='self-start'>
                <div className="grid grid-cols-2">
                    <p>운항사</p>
                    <p className='text-end'> {airlines.find((airline) => airline.airlineCode == flightInfo.flightNo.substring(0, 2)).name}</p>
                </div>
                <div className="grid grid-cols-2">
                    <p>실제 운항사</p>
                    <p className='text-end'>{airlines.find((airline) => airline.airlineCode == flightInfo.realFlightNo.substring(0, 2)).name}</p>
                </div>
                <div className="grid grid-cols-2">
                    <p>셀프 체크인 / 백드랍</p>
                    {/* 밑에 수정 */}
                    {airlines.find((airline) => airline.airlineCode == flightInfo.realFlightNo.substring(0, 2)).selfCheckIn ?
                        <p className="text-end text-blue-600">가능</p> :
                        <p className="text-end text-red-600">불가능</p>
                    }
                    <P></P>
                </div>
            ...
            <Link to="/info/terminaltransport">
                <IconAndP
                    className="flex flex-row justify-center mb-4 my-5"
                    svg={pointerIcon}
            ...
            <div className='self-start'>
                ...
                <P>
                    공동운항 여부에 따라 예약한 항공사와 실제 운항하는 항공사가 다른 경우가 있어요. 이 경우에 해당편 수속은 실제 운항 항공사 카운터에서 담당하게 됩니다.
                </P>
            </div>
        </div>
    );
